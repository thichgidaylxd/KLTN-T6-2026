...
import { useHarvestsByStage, useHarvestSummaryByStage } from '@/hooks/harvest/useHarvest';
...
export const HarvestByStagePage: React.FC<HarvestByStagePageProps> = ({ 
  planId: propPlanId, 
  stageId: propStageId, 
  stageName: propStageName 
}) => {
  const { planId: paramPlanId, stageId: paramStageId, stageName: paramStageName } = useParams<{ 
    planId: string; 
    stageId: string; 
    stageName: string 
  }>();
  ...
  const stageId = propStageId || paramStageId || '';
  const stageName = propStageName || paramStageName || '';
  
  const [page, setPage] = useState(0);
  const { data: harvests, isLoading: harvestLoading } = useHarvestsByStage(planId, stageId, page, 10);
  const { data: summaryData, isLoading: summaryLoading } = useHarvestSummaryByStage(planId, stageId);
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 bg-slate-50/50 min-h-[calc(100vh-4rem)]">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Thu hoạch theo giai đoạn</h1>
        <p className="text-slate-500 font-medium">{stageName}</p>
      </div>
              ...
              {harvestList.length > 0 ? (
                harvestList.map((harvest: any) => (
                  <tr key={harvest.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 text-sm font-semibold text-slate-700">{harvest.plotName}</td>
